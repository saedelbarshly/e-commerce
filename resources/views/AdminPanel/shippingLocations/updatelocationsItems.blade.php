  ...
  <div class="options-list-place">
    <div class="row  options-list">
      <div class="col-12 col-md-4 mb-1">
        ...
        <select name="countryID[]" id="countryID" class="form-control option-countryID" required>
          <option value="0">كل الدول</option>
          @foreach($countries as $country)
          <option value="{{$country->id}}" {{ ($item->country_id == $country->id)? 'selected' : '' }}>{{$country->name_ar}}</option>
          @endforeach
        </select>
      </div>
      <div class="col-12 col-md-4 mb-1">
        <label class="form-label" for="governorateID">{{trans('common.governorates')}}</label>
        <select name="governorateID[]" id="governorateID" class="form-control option-governorates" required>
          <option value="0">كل المحافظات</option>
          <option value="{{$item->governorate_id}}" selected>{{$item->governorate_id}}</option>
        </select>
      </div>
      <div class="col-12 col-md-3 mb-1">
        ...
        <select name="cityID[]" id="cityID" class="form-control option-cityID" required>
          ...
          <option value="{{$item->city_id}}" selected>{{$item->city_id}}</option>
        </select>
      </div>
